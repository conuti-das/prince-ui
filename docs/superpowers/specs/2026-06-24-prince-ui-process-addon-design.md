...
- `~/dev/MacoAi/finops/frontend/src/components/ProcessInstances/BpmnDiagram.tsx` — Status/Historie-Highlighting (~1400 Z., fragiles imperatives SVG-Recoloring, an `data-ui5-theme` gekoppelt). **Wird durch ① abgelöst.**
...
**Zweck:** Task-Formulare schema-getrieben in prince-ui-Optik rendern (löst UI5-Primitive in finops `UserTaskFormDialog.tsx` ab).
...
- **Theme-Detection vereinheitlichen:** überall `data-theme` + `--prn-*` (nicht `data-ui5-theme`/`data-sap-theme`).
